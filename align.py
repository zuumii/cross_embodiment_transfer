...
def _joint_point_world(sim, joint_id):
    # 关节 anchor 点世界坐标
    m, d = _unwrap_mj(sim)
    jid = int(joint_id)
...
def _analytic_point_jac(sim, joint_id, arm_vel_cols):
    # Jp: (3, dof_arm) 点的线速度雅可比（列为 qvel 顺序）
    m, d = _unwrap_mj(sim)
    bid = int(m.jnt_bodyid[joint_id])
    p_world = _joint_point_world(sim, joint_id).astype(np.float64)
...
class _SEWFromQ_MJ(torch.autograd.Function):
    ...
    def forward(ctx, q, sim, arm_qpos_addr, arm_vel_cols, sew_jids, ref_dir=(0.0, 1.0, 0.0)):
        ...
        # 写姿态并 forward
        sim.data.qpos[np.array(arm_qpos_addr, dtype=int)] = q_np
        sim.forward()

        jid_S, jid_E, jid_W = sew_jids
        S_np = _joint_point_world(sim, jid_S)
        E_np = _joint_point_world(sim, jid_E)
        W_np = _joint_point_world(sim, jid_W)
        ...
        # MuJoCo 解析雅可比
        J_S = torch.from_numpy(_analytic_point_jac(sim, jid_S, arm_vel_cols)).to(device=device, dtype=dtype)
        J_E = torch.from_numpy(_analytic_point_jac(sim, jid_E, arm_vel_cols)).to(device=device, dtype=dtype)
        J_W = torch.from_numpy(_analytic_point_jac(sim, jid_W, arm_vel_cols)).to(device=device, dtype=dtype)
...
def sigma_from_q_once(sim, q_tensor, arm_qpos_addr, arm_vel_cols, sew_jids, ref_dir=(0.0, 1.0, 0.0)):
    # q_tensor 可是计算图的“视图”（比如来自 decoder 输出的切片），梯度会正确回传
    return _SEWFromQ_MJ.apply(q_tensor, sim, arm_qpos_addr, arm_vel_cols, sew_jids, ref_dir)
...
class ObsAligner:
    ...
    def _sigma_batch(self, which_env, Q):
        ...
        sim = cfg["sim"]; arm_qpos_addr = cfg["arm_qpos_addr"]; arm_vel_cols = cfg["arm_vel_cols"]
        ...
        for i in range(Q.shape[0]):
            q_i = Q[i]
            # 用 double 稳定数值，grad 会正确回传
            q_i64 = q_i.to(torch.float64)
            sigma_i = sigma_from_q_once(sim, q_i64, arm_qpos_addr, arm_vel_cols, sew_jids, ref_dir=self.ref_dir)
            # 返回 float32 以避免和网络 dtype 冲突
